...
void schedule()
{
	...
	if(getcontext(&scheduler_context.context) == -1) {
...
void my_pthread_create_init(void)
...
{ 	// create scheduler thread
    ...
    scheduler_context.uc_link = &maincontext;

    /*Saving context for main thread*/
	scheduler_context.id = -1;
	if ( getcontext(&(scheduler_context.uc)) == -1) {
		printf("Error getting scheduler context\n");
		exit(EXIT_FAILURE);
	}	
	CURRENT = &scheduler_context; /* Storing scheduler thread as current*/
	g_threads_cnt ++;
	makecontext(&scheduler_context.context, scheduler, NULL);
...
int my_pthread_create(my_pthread_t * thread, pthread_attr_t * attr, void *(*function)(void*), void * arg) {
	...
	ti->context->uc_link = &scheduler_context;
